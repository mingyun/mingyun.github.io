...
```
###[微博床图](https://segmentfault.com/q/1010000008250059)
```js
用 http://picupload.service.weibo.com/interface/pic_upload.php?mime=image%2Fjpeg&data=base64&url=0&markpos=1&logo=&nick=0&marks=1&app=miniblog 这个 URL 的话
...
$post_data['pic1']=curl_file_create('d:/gg.jpg', 'image/jpeg', "gg.jpg");
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,"http://picupload.service.weibo.com/interface/pic_upload.php?mime=image%2Fjpeg&data=base64&url=0&markpos=1&logo=&nick=0&marks=1&app=miniblog&cb=http://weibo.com/aj/static/upimgback.html?_wv=5&callback=STK_ijax_".time());
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_COOKIEFILE, $cookie); 
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
...
```
###[supervisor集中式管理](http://www.jianshu.com/p/eaef9d040e9e)
```js
...
```
###[python模拟登录微博](https://github.com/trytofix/hexo_weibo_image/blob/master/weibo_util.py)
```js
# encoding:utf8
...
```
###[Python二级域名信息收集工具](https://www.waitalone.cn/python-subdomain.html)
```js
#!/usr/bin/env python
# -*- coding: gbk -*-
# -*- coding: utf-8 -*-
